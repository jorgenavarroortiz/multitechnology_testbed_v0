...
def get_mptcp_telemetry(subflows, netns=None):
    telemetry = []
    ...
    netnscall=""
    if netns!=None:
        netnscall="ip netns exec "+netns+" "
    stream = os.popen(netnscall+"ss -nite \"" + filters + "\"")
...
def get_mptcp_subflows_from_inode(inode,netns=None):
    socket_list = []

    netnscall=["cat","/proc/net/tcp"]
    if netns!=None:
        netnscall=["ip","netns","exec",netns]+netnscall

    process  = subprocess.run(netnscall,universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def get_mptcp_socket_scheduler(inode,netns=None):
    scheduler=None
    
    socket=get_mptcp_socket(inode,netns)
    if socket!=None:
        scheduler=socket["scheduler"]

    return scheduler

def get_mptcp_socket(inode,netns=None):
    socket=None
    socket_list= get_mptcp_sockets(netns)
...
def get_mptcp_sockets(netns=None):
    socket_list = []

    netnscall=["cat","/proc/net/mptcp_net/mptcp"]
    if netns!=None:
        netnscall=["ip","netns","exec",netns]+netnscall
