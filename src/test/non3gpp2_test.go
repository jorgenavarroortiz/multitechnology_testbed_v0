...
// Parsing arguments passed to Test methods. 
var UeIndex = flag.Int("UeIndex", 1, "UE index when creating N UEs") //For TestI2catNon3GPP

func createIKEChildSecurityAssociation2(chosenSecurityAssociation *ike_message.SecurityAssociation) (*n3iwf_context.ChildSecurityAssociation, error) {
...
func setupUDPSocket2(t *testing.T, IpAddress string) *net.UDPConn {
        port := ":500"
	bindAddr := IpAddress + port
...
func concatenateNonceAndSPI2(nonce []byte, SPI_initiator uint64, SPI_responder uint64) []byte {
	spi := make([]byte, 8)
...
func generateKeyForIKESA2(ikeSecurityAssociation *n3iwf_context.IKESecurityAssociation) error {
	// Transforms
	...
	seed := concatenateNonceAndSPI2(ikeSecurityAssociation.ConcatenatedNonce, ikeSecurityAssociation.LocalSPI, ikeSecurityAssociation.RemoteSPI)
...
func generateKeyForChildSA2(ikeSecurityAssociation *n3iwf_context.IKESecurityAssociation, childSecurityAssociation *n3iwf_context.ChildSecurityAssociation) error {
	// Transforms
	transformPseudorandomFunction := ikeSecurityAssociation.PseudorandomFunction
...
func decryptProcedure2(ikeSecurityAssociation *n3iwf_context.IKESecurityAssociation, message *ike_message.IKEMessage, encryptedPayload *ike_message.Encrypted) ([]ike_message.IKEPayloadType, error) {
...
func encryptProcedure2(ikeSecurityAssociation *n3iwf_context.IKESecurityAssociation, ikePayload []ike_message.IKEPayloadType, responseIKEMessage *ike_message.IKEMessage) error {
	// Load needed information
	transformIntegrityAlgorithm := ikeSecurityAssociation.IntegrityAlgorithm
	transformEncryptionAlgorithm := ikeSecurityAssociation.EncryptionAlgorithm
	checksumLength := 12 // HMAC_SHA1_96
...
func buildEAP5GANParameters2() []byte {
...
func parseIPAddressInformationToChildSecurityAssociation2(
	...
	childSecurityAssociation.LocalPublicIPAddr = net.ParseIP(LocalIpAddr)
...
func applyXFRMRule2(ue_is_initiator bool, childSecurityAssociation *n3iwf_context.ChildSecurityAssociation) error {
...
func TestI2catNon3GPPUE(t *testing.T) {
	// New UE
        var UeImsi string
	UeImsi = "imsi-208930000748" + strconv.Itoa(*UeIndex)
        t.Logf("UeImsi is: %v\n", UeImsi)
//	ue := NewRanUeContext("imsi-2089300007487", 1, ALG_CIPHERING_128_NEA0, ALG_INTEGRITY_128_NIA0)
	ue := NewRanUeContext(UeImsi, 1, ALG_CIPHERING_128_NEA0, ALG_INTEGRITY_128_NIA0)
//	ue.AmfUeNgapId = 1
	ue.AmfUeNgapId = int64(*UeIndex)
	ue.AuthenticationSubs = getAuthSubscription2()
	mobileIdentity5GS := nasType.MobileIdentity5GS{
		Len:    12, // suci
		Buffer: []uint8{0x01, 0x02, 0xf8, 0x39, 0xf0, 0xff, 0x00, 0x00, 0x00, 0x00, 0x47, 0x78}, // TODO: Correct SUCI per UE
	}

	n3iwfUDPAddr, err := net.ResolveUDPAddr("udp", "192.168.13.2:500")
	if err != nil {
		t.Fatal(err)
	}

	var LocalIpAddress string
        LocalIpAddress = "192.168.13." + strconv.Itoa(*UeIndex + 2)
        if LocalIpAddress == "" {
		t.Logf("Variable passing did not work\n")
	} else {
		t.Logf("Local IP address for the IPSEC tunnel is: %v\n", LocalIpAddress)
	}
	udpConnection := setupUDPSocket2(t, LocalIpAddress)

        fmt.Println("\n ########### Opening Transport UDP socket ############### \n")

	// IKE_SA_INIT
	var UeInitiatorSPI uint64
	UeInitiatorSPI=uint64(123122 + *UeIndex)
	fmt.Println("\n ####### UeInitiatorSPI: ", UeInitiatorSPI)
//	ikeMessage := ike_message.BuildIKEHeader(123123, 0, ike_message.IKE_SA_INIT, ike_message.InitiatorBitCheck, 0)
	ikeMessage := ike_message.BuildIKEHeader(UeInitiatorSPI, 0, ike_message.IKE_SA_INIT, ike_message.InitiatorBitCheck, 0)
...
//		LocalSPI:               123123,
	...
	if err := generateKeyForIKESA2(ikeSecurityAssociation); err != nil {
...
//	ikeMessage = ike_message.BuildIKEHeader(123123, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 1)
	ikeMessage = ike_message.BuildIKEHeader(UeInitiatorSPI, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 1)

	var ikePayload []ike_message.IKEPayloadType

	// Identification
        var IdthisUe string
	IdthisUe = "UE" + strconv.Itoa(*UeIndex)
//	identification := ike_message.BuildIdentificationInitiator(ike_message.ID_FQDN, []byte("UE"))
	identification := ike_message.BuildIdentificationInitiator(ike_message.ID_FQDN, []byte(IdthisUe))
	ikePayload = append(ikePayload, identification)

        fmt.Println("\n ### IKE identification ", identification)

	// Security Association
        bs := make([]byte, 4)
        binary.LittleEndian.PutUint32(bs, uint32(*UeIndex))
	proposal = ike_message.BuildProposal(1, ike_message.TypeESP, bs)

/*        if *UeIndex == 1 {
		proposal = ike_message.BuildProposal(1, ike_message.TypeESP, []byte{0, 0, 0, 1})
	}
        if *UeIndex == 2 {
		proposal = ike_message.BuildProposal(1, ike_message.TypeESP, []byte{0, 0, 0, 2})
	}
*/
        fmt.Println("\n ### IKE_AUTH Proposal ", proposal) // This is what seems to fail for the seconfd UE ike_handle/handler.go --> Handle_IKEAUTH does not find a proposal and generated WARNING
	encryptTransform = ike_message.BuildTransform(ike_message.TypeEncryptionAlgorithm, ike_message.ENCR_AES_CBC, &attributeType, &keyLength, nil)
	ike_message.AppendTransformToProposal(proposal, encryptTransform)
	integrityTransform = ike_message.BuildTransform(ike_message.TypeIntegrityAlgorithm, ike_message.AUTH_HMAC_SHA1_96, nil, nil, nil)
	ike_message.AppendTransformToProposal(proposal, integrityTransform)
	...
	if err := encryptProcedure2(ikeSecurityAssociation, ikePayload, ikeMessage); err != nil {
	...
	decryptedIKEPayload, err := decryptProcedure2(ikeSecurityAssociation, ikeMessage, encryptedPayload)
	if err != nil {
...
//	ikeMessage = ike_message.BuildIKEHeader(123123, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 2)
	ikeMessage = ike_message.BuildIKEHeader(UeInitiatorSPI, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 2)
	...
	// AN Parameters
	anParameters := buildEAP5GANParameters2()
...
//	ikeMessage = ike_message.BuildIKEHeader(123123, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 3)
	ikeMessage = ike_message.BuildIKEHeader(UeInitiatorSPI, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 3)
	...
	err = encryptProcedure2(ikeSecurityAssociation, ikePayload, ikeMessage)
	if err != nil {
		t.Fatal(err)
	}


        fmt.Println("\n ########### Send IKE_AUTH + EAP-5G - 2 ############### \n")
...
//	ikeMessage = ike_message.BuildIKEHeader(123123, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 4)
	ikeMessage = ike_message.BuildIKEHeader(UeInitiatorSPI, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 4)
...
//	ikeMessage = ike_message.BuildIKEHeader(123123, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 5)
	ikeMessage = ike_message.BuildIKEHeader(UeInitiatorSPI, ikeSecurityAssociation.RemoteSPI, ike_message.IKE_AUTH, ike_message.InitiatorBitCheck, 5)
	...
	childSecurityAssociationContext, err := createIKEChildSecurityAssociation2(ikeSecurityAssociation.IKEAuthResponseSA)
	if err != nil {
		t.Fatalf("Create child security association context failed: %+v", err)
		return
	}
	err = parseIPAddressInformationToChildSecurityAssociation2(childSecurityAssociationContext, net.ParseIP("192.168.13.2"), responseTrafficSelectorInitiator.TrafficSelectors[0], responseTrafficSelectorResponder.TrafficSelectors[0], LocalIpAddress)
	if err != nil {
	...
	if err := generateKeyForChildSA2(ikeSecurityAssociation, childSecurityAssociationContext); err != nil {
		t.Fatalf("Generate key for child SA failed: %+v", err)
		return
	}

	// Aplly XFRM rules
	if err = applyXFRMRule2(true, childSecurityAssociationContext); err != nil {
...
//	}()
	...
	childSecurityAssociationContextUserPlane, err := createIKEChildSecurityAssociation2(responseSecurityAssociation)
	if err != nil {
		t.Fatalf("Create child security association context failed: %+v", err)
		return
	}
	err = parseIPAddressInformationToChildSecurityAssociation2(childSecurityAssociationContextUserPlane, net.ParseIP("192.168.13.2"), responseTrafficSelectorResponder.TrafficSelectors[0], responseTrafficSelectorInitiator.TrafficSelectors[0], LocalIpAddress)
	if err != nil {
		t.Fatalf("Parse IP address to child security association failed: %+v", err)
		return
	}
	...
	if err := generateKeyForChildSA2(ikeSecurityAssociation, childSecurityAssociationContextUserPlane); err != nil {
		t.Fatalf("Generate key for child SA failed: %+v", err)
		return
	}
	...
	// Aplly XFRM rules
	if err = applyXFRMRule2(false, childSecurityAssociationContextUserPlane); err != nil {
		t.Fatalf("Applying XFRM rules failed: %+v", err)
		return
	}
...
//        tapMac, err := net.ParseMAC("42:00:00:00:00:01")
//	newGRETunnel := &netlink.Gretap{
//		LinkAttrs: netlink.LinkAttrs{
//			Name: "gretap0",
//		        HardwareAddr: tapMac,
//		},
