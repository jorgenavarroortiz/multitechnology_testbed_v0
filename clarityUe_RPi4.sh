...
usage() {
  ...
  echo ""
  echo "E.g.: $0 -n 2 -m -P fullmesh -S default -C olia -s 10.0.1 -o 60.60.0.101 -i eth4 -I 60.60.0.33/24"
  echo ""
  ...
  echo "       -h ....................... this help"
  echo ""
  echo "Please remember to update the file if_names.txt according to your needs. This file includes the names of the network interfaces that will be used for MPTCP (one row per interface). It should be 'eth0' and 'wlan0' in the case of a Raspberry Pi 4."
  echo "Please note that 5G attach has not been tested for this architecture and it is disabled."
  exit 1;
}
...
while getopts ":n:mP:S:C:c:s:o:i:I:dh" o; do
  case "${o}" in
    n)
      NUM_UES=${OPTARG}
      n=1
...
######################################
# DISABLED for Raspberry Pi 4 (not compiled nor tested)
if [ ${ATTACH} ]
then
  for i in $(seq 1 $NUM_UES)
  do
